...
import my.dialy.dairy.journal.dairywithlock.databinding.FragmentJunBinding

@AndroidEntryPoint
class JunFragment : Fragment() {
    ...
    lateinit var binding: FragmentJunBinding
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        binding= FragmentJunBinding.inflate(layoutInflater,container,false)
        getData()
    ...
    private fun setEmoji(result: List<NoteViewModelClass>) {
        if (allDates.isNotEmpty()) {
            for (data in result) {
                if (data.monthString == "Jun" && data.year == "2024") {
    ...
    private fun setInitialBackground() {
        try {
            ...
            if (currentDate!=null && currentMonth=="Jun") {
    ...
    private fun onDateSelected(textView: TextView?) {
        textView?.let {
            ...
            coordinatorViewModel.sendMonthToViewModel("Jun")
