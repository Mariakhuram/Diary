...
import my.dialy.dairy.journal.dairywithlock.databinding.FragmentNovBinding

@AndroidEntryPoint
class NovFragment : Fragment() {
    ...
    lateinit var binding: FragmentNovBinding
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        binding= FragmentNovBinding.inflate(layoutInflater,container,false)
        getData()
    ...
    private fun setEmoji(result: List<NoteViewModelClass>) {
        if (allDates.isNotEmpty()) {
            for (data in result) {
                if (data.monthString == "Nov" && data.year == "2024") {
    ...
    private fun setInitialBackground() {
        try {
            ...
            if (currentDate!=null && currentMonth=="Nov") {
    ...
    private fun onDateSelected(textView: TextView?) {
        textView?.let {
            ...
            coordinatorViewModel.sendMonthToViewModel("Nov")
