...
import my.dialy.dairy.journal.dairywithlock.databinding.FragmentMayBinding
...
class MayFragment : Fragment() {
    ...
    lateinit var binding: FragmentMayBinding
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        binding= FragmentMayBinding.inflate(layoutInflater,container,false)
        getData()
    ...
    private fun setEmoji(result: List<NoteViewModelClass>) {
        if (allDates.isNotEmpty()) {
            for (data in result) {
                if (data.monthString == "May" && data.year == "2024") {
    ...
    private fun setInitialBackground() {
        try {
            ...
            if (currentDate!=null && currentMonth=="May") {
    ...
    private fun onDateSelected(textView: TextView?) {
        textView?.let {
            ...
            coordinatorViewModel.sendMonthToViewModel("May")
