...
import my.dialy.dairy.journal.dairywithlock.databinding.FragmentDecBinding
...
class DecFragment : Fragment() {
    ...
    lateinit var binding: FragmentDecBinding
    ...
    override fun onCreateView(
        ...
        binding= FragmentDecBinding.inflate(layoutInflater,container,false)
        getData()
    ...
    private fun setEmoji(result: List<NoteViewModelClass>) {
        if (allDates.isNotEmpty()) {
            for (data in result) {
                if (data.monthString == "Dec" && data.year == "2023") {
    ...
    private fun setInitialBackground() {
        try {
            ...
            if (currentDate!=null && currentMonth=="Dec") {
    ...
    private fun onDateSelected(textView: TextView?) {
        textView?.let {
            ...
            coordinatorViewModel.sendMonthToViewModel("Dec")
            coordinatorViewModel.sendYearToViewModel("2023")
