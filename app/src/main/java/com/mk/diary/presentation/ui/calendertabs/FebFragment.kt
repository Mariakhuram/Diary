...
import my.dialy.dairy.journal.dairywithlock.databinding.FragmentFebBinding

@AndroidEntryPoint
class FebFragment : Fragment() {
    ...
    lateinit var binding: FragmentFebBinding
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentFebBinding.inflate(layoutInflater, container, false)
    ...
    private fun setEmoji(result: List<NoteViewModelClass>) {
        if (allDates.isNotEmpty()) {
            for (data in result) {
                if (data.monthString == "Feb" && data.year == "2024") {
    ...
    private fun setInitialBackground() {
        try {
            ...
            if (currentDate!=null && currentMonth=="Feb") {
    ...
    private fun onDateSelected(textView: TextView?) {
        textView?.let {
            ...
            coordinatorViewModel.sendMonthToViewModel("Feb")
...
// Set the modified background back to the TextView
    ...
    private fun resetSelectedDateBackground() {
        selectedDateTextView?.setBackgroundResource(R.drawable.rounded_background)
    }
