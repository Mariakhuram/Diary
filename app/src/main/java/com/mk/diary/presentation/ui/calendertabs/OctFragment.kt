...
import my.dialy.dairy.journal.dairywithlock.databinding.FragmentOctBinding

@AndroidEntryPoint
class OctFragment : Fragment() {
    ...
    lateinit var binding: FragmentOctBinding
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        binding= FragmentOctBinding.inflate(layoutInflater,container,false)
        getData()
    ...
    private fun setEmoji(result: List<NoteViewModelClass>) {
        if (allDates.isNotEmpty()) {
            for (data in result) {
                if (data.monthString == "Oct" && data.year == "2024") {
    ...
    private fun setInitialBackground() {
        try {
            ...
            if (currentDate!=null && currentMonth=="Oct") {
    ...
    private fun onDateSelected(textView: TextView?) {
        textView?.let {
            ...
            coordinatorViewModel.sendMonthToViewModel("Oct")
